"""Bike-share network planning and optimization utilities."""
...
class BikeShareOptimizer:
    ...
    def predict_rider_flow(self, new_stations=None):
        ...
        trip_counts = (
            self.trip_data.groupby(["start_station", "end_station"]).size().reset_index(name="trips")
        )
    ...
    def assess_station_poi_proximity(self, random_state=42):
        """Generate demonstration POI scores when external POI data is unavailable."""
    ...
    def recommend_station_capacity(self):
        ...
        peak["recommended_capacity"] = np.ceil(peak["max_flow"] * 0.2).astype(int).clip(lower=10)
